...
import { UserAnswer, Answer } from "@/types/answers.types";
...
function classQuestionStateItem(i:number) {
  let basic = "border border-gray-80 h-8 w-8 flex justify-center items-center m-2 hover:cursor-pointer duration-300 ";

  if (!triggerShowAnswer.value) {
    if (i == questionIndex.value)
      return basic + "text-white bg-blue-400";
    if (!userSelects.value || !userSelects.value[i] || userSelects.value[i] == '')
      return basic + "hover:bg-gray-100";
    else
      return basic + "bg-blue-200 opacity-40 hover:bg-blue-400";
  } else {
    if (i == questionIndex.value) {
      if (answers[i].correct)
        return basic + "text-white bg-green-600";
      else
        return basic + "text-white bg-red-600";
    } else {
      if (answers[i].correct)
        return basic + "bg-green-200 opacity-40 hover:bg-green-500";
      else
        return basic + "bg-red-200 opacity-40 hover:bg-red-400"
    }
  }
}
function classSelectId(id: string) {
  ...
  if (!triggerShowAnswer.value) {
    ...
    if (answers[questionIndex.value].answer == id)
      return basic + "bg-green-600 text-white";
    if (userSelects.value[questionIndex.value] == id)
      return basic + "bg-red-500 text-white";
    return basic + "text-blue-500";
  }
}
  ...
  <div class="flex flex-col sm:flex-row sm:mx-10">
    ...
    <div class="flex order-12 sm:order-1 flex-col sm:basis-2/3 bg-white m-4 shadow-lg relative" >
      ...
      <div class=" flex flex-1 flex-col border-b">
        ...
        <div class="flex flex-1 flex-col py-4 px-8 bg-gray-50"
             v-if="triggerShowAnswer">
          <div>
      ...
      <div class=" flex justify-center space-x-8">
        <RoomQuestionButton value="上一题"
                            v-show="questionIndex > 0" @click="handleQuestionId({type: 'step', id: -1})"/>
        <RoomQuestionButton value="下一题"
                            v-show="questionIndex < questions.length - 1" @click="handleQuestionId({type: 'step', id: 1})"/>
        ...
        <RoomQuestionButton value="清除"
                            v-show="userSelects[questionIndex] && !triggerShowAnswer" @click="userSelects[questionIndex] = null;"/>
      </div>
      ...
      <div class="absolute w-full h-full top-0 left-0 flex items-center justify-center bg-gray-300 opacity-80"
        ...
        <!-- 提交确认框 -->
        <div class="flex w-1/3 bg-white shadow-lg flex-col rounded-xl"
          ...
          <div class="flex flex-col sm:flex-row items-center border-t border-gray-300">
            <div class="flex-1 flex justify-center py-2 hover:cursor-pointer hover:bg-gray-100 duration-300 active:bg-gray-300"
                 @click="handleUserSelects({type: 'submit'})">确定</div>
        ...
        <div class="flex bg-white flex-col p-5 space-y-4 rounded-lg justify-center items-center"
          ...
          <div class="flex space-x-2 flex-row">
            ...
            <div class="p-2 w-4 h-4 rounded-full animate-bounce delay-75 bg-yellow-500" style="animation-delay: 0.1s"></div>
            <div class="p-2 w-4 h-4 rounded-full animate-bounce delay-150 bg-green-600" style="animation-delay: 0.2s"></div>
          </div>
        </div>

        <div class="bg-white rounded-lg p-4"
            v-show="windowPauseTime">
          暂停中...
        </div>
      </div>
    </div>
    ...
    <div class=" order-1 sm:order-12 flex-1 bg-white m-4 flex-col" >
      <div class=" shadow-lg mb-10">
        <div class=" flex flex-row p-2 bg-blue-400 text-white">
          ...
          <div v-show="!triggerSubmitSelects" class="my-2 md:my-0 md:mx-2 basis-1/4 flex flex-col items-center hover:cursor-pointer"
      ...
      <div class=" flex flex-col shadow-lg">
        ...
        <div class=" flex flex-wrap content-between max-h-80 overflow-scroll px-2 justify-center">
          <template v-for="(item, i) in questions" :key="i">
            ...
            <div :class="classQuestionStateItem(i)"
